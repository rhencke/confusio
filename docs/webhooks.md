...
have no equivalent in any self-hosted forge.  Confusio does not emit these events from
any backend today; future work will synthesize `installation` and
`installation_repositories` events on startup from the configured backend.

| GitHub event | Status | Reason |
|---|---|---|
| `installation` | ✗ | GitHub App installation lifecycle (created, deleted, suspend, unsuspend, new_permissions_accepted). No self-hosted forge has an App installation model. |
| `installation_repositories` | ✗ | Repositories added to or removed from a GitHub App installation. No cross-forge equivalent. |
...
or when the user accepts new permissions requested by the app.
