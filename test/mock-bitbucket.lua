...
function OnHttpRequest()
  ...
  local REPO =
    '{"uuid":"{1234}","slug":"hello-world","name":"Hello-World",' ..
  ...
  elseif path == rb .. "/refs/tags" then
    SetStatus(200, "OK")
    json('{"values":[{"name":"v1.0","target":{"hash":"abc123def456","type":"commit"}}],' ..
  ...
  elseif path == rb .. "/refs/branches" then
    SetStatus(200, "OK")
    json('{"values":[{"name":"main","target":{"hash":"abc123def456","type":"commit"},' ..
      '"type":"branch"}],"pagelen":30,"size":1,"page":1}')

  -- Commits ----------------------------------------------------------------
  elseif path == rb .. "/commit/abc123" then
    SetStatus(200, "OK")
    json('{"hash":"abc123def456","message":"Initial commit","date":"2011-01-26T19:01:12Z",' ..
  ...
  elseif path == rb .. "/commits" then
    SetStatus(200, "OK")
    json('{"values":[{"hash":"abc123def456","message":"Initial commit",' ..
  ...
  elseif path == rb .. "/commit/abc123/statuses" then
    SetStatus(200, "OK")
    json('{"values":[{"state":"SUCCESSFUL","key":"ci","description":"Build passed",' ..
      '"url":"http://ci.example.com","created_on":"2020-01-01T00:00:00Z",' ..
      '"updated_on":"2020-01-01T00:00:00Z"}],"pagelen":30,"size":1,"page":1}')

  -- Contents ---------------------------------------------------------------
  elseif path == rb .. "/src/main/README.md" or
         path == rb .. "/src/HEAD/README.md" then
    SetStatus(200, "OK")
    raw("# Hello World\n")
  ...
  elseif path == rb .. "/deploy-keys/1" then
    SetStatus(200, "OK")
    json('{"id":1,"key":"ssh-rsa AAAAB3...","label":"my key",' ..
      '"created_on":"2020-01-01T00:00:00Z"}')

  elseif path == rb .. "/deploy-keys" then
    SetStatus(200, "OK")
    json('{"values":[{"id":1,"key":"ssh-rsa AAAAB3...","label":"my key",' ..
