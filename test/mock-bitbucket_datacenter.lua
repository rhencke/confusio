...
function OnHttpRequest()
  ...
  local REPO =
    '{"id":1,"slug":"hello-world","name":"Hello World","scmId":"git",' ..
  ...
  elseif path == rb .. "/ssh/1" then
    SetStatus(200, "OK")
    json('{"id":1,"key":{"id":1,"text":"ssh-rsa AAAAB3...","label":"my key",' ..
      '"createdDate":1609459200000}}')

  elseif path == rb .. "/ssh" then
    SetStatus(200, "OK")
    json('{"values":[{"id":1,"key":{"id":1,"text":"ssh-rsa AAAAB3...",' ..
      '"label":"my key","createdDate":1609459200000}}],"isLastPage":true}')

  -- Webhooks ----------------------------------------------------------------
  elseif path == rb .. "/webhooks/1" then
    SetStatus(200, "OK")
    json('{"id":1,"name":"web","url":"https://example.com/hook",' ..
