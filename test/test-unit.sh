...
# Secrets are declared via webhook_secret_BACKEND=SECRET CLI args (no env vars).
...
WH_CLI_ARGS="-- webhook_secret_gitea=$WH_SECRET webhook_secret_gitlab=$WH_SECRET webhook_secret_bitbucket=$WH_SECRET webhook_secret_gitbucket=$WH_SECRET webhook_secret_confusio=$WH_SECRET"
wh_dir=$(mktemp -d)
start_confusio "$wh_dir" "$WH_CLI_ARGS"; WH_PID=$!
trap "kill $WH_PID 2>/dev/null || true; rm -rf $wh_dir" EXIT
$HURL --retry 10 --retry-interval 200 --connect-timeout 1 --max-time 5 \
