...
validate-builders:
	...
	for f in backends/*.lua; do \
	  if ! grep -q ':build(' "$$f" && ! grep -q ':spec(' "$$f" && ! grep -q 'dofile' "$$f"; then \
	    bad="$$bad $$f"; \
	  fi; \
	done; \
	if [ -n "$$bad" ]; then \
	  echo "ERROR: backend(s) missing b:build()/b:spec() call (and not an alias that dofiles a root backend):$$bad" >&2; \
...
test-unit: test-unit-functions gen-webhook-fixtures confusio.com $(MOCKS) mock-target.com hurl
