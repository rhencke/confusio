...
local base = function() return config.base_url .. "/a" end
...
backend_impl = {
  ...
  patch_repo = function(owner, repo_name)
    ...
    local g = {}
    if req.description then g.description = req.description end
    proxy_json(
      function(r) return translate_gerrit_repo(r, owner, repo_name) end,
      fetch_json(base() .. "/projects/" .. project_id(owner, repo_name) .. "/config",
        "PUT", EncodeJson(g)))
  end,
    ...
    proxy_json(
      function(data)
        ...
        for name, r in pairs(data or {}) do
          r.name = name; repos[#repos + 1] = translate_gerrit_repo(r)
        end
        return repos
      end,
      fetch_json(url))
  ...
  get_users_repos = function(username)
    local limit = GetParam("per_page") or "30"
    local skip  = ((tonumber(GetParam("page")) or 1) - 1) * (tonumber(limit) or 30)
    local url = base() .. "/projects/?p=" .. username .. "%2F&n=" .. limit
