...
backend_impl = {
  ...
  -- GitLab has a per-project export API (POST /projects/:id/export,
  -- GET /projects/:id/export/download) but no org- or user-level migration concept
  -- that maps to GitHub's stateful multi-repo model.  Because confusio is a stateless
  -- proxy it cannot track migration IDs across requests, so all migration endpoints
  -- return the same fixed responses as the global defaults, listed explicitly here.
