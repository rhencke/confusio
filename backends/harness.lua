...
local function translate_harness_commit(c)
  ...
  return {
    ...
    commit = {
      ...
      committer = { name = cident.name or "", email = cident.email or "", date = committer.when or "" },
    },
...
backend_impl = {
  ...
  get_repo_branches = function(owner, repo_name)
    proxy_json(
      function(branches)
        ...
        for i, b in ipairs(branches) do branches[i] = translate_harness_branch(b) end
        return branches
      end,
      fetch_json(append_page_params(
  ...
  get_repo_commits = function(owner, repo_name)
    ...
    proxy_json(
      function(commits)
        commits = commits or {}
        for i, c in ipairs(commits) do commits[i] = translate_harness_commit(c) end
        return commits
      end,
      fetch_json(url))
  end,
  ...
  post_commit_status = function(owner, repo_name, sha)
    proxy_json_created(nil,
      fetch_json(base() .. "/repos/" .. repo_ref(owner, repo_name) .. "/check/commits/" .. sha,
        "POST", GetBody()))
  end,
  ...
  get_repo_content = function(owner, repo_name, path)
    ...
    proxy_json(
      function(data)
        ...
        if data and data.type == "dir" then return data.entries or {} end
        return data or {}
      end,
      fetch_json(url))
  end,
  ...
  put_repo_content = function(owner, repo_name, path)
    proxy_json(nil,
      fetch_json(base() .. "/repos/" .. repo_ref(owner, repo_name) .. "/content/" .. path,
        "PUT", GetBody()))
  end,
  ...
  post_repo_forks = function(owner, repo_name)
    proxy_json_created(translate_harness_repo,
      fetch_json(base() .. "/repos/" .. repo_ref(owner, repo_name) .. "/fork",
        "POST", GetBody()))
  end,
  ...
  get_repo_keys = function(owner, repo_name)
    proxy_json(
      function(keys)
        keys = keys or {}
        for i, k in ipairs(keys) do keys[i] = translate_harness_key(k) end
        return keys
      end,
      fetch_json(append_page_params(
  ...
  post_repo_keys = function(owner, repo_name)
    local req = DecodeJson(GetBody() or "{}")
    local h = { identifier = req.title or "", public_key = req.key or "",
                usage = req.read_only and "read" or "readwrite" }
    proxy_json_created(translate_harness_key,
      fetch_json(base() .. "/repos/" .. repo_ref(owner, repo_name) .. "/keys",
        "POST", EncodeJson(h)))
  end,
  ...
  delete_repo_key = function(owner, repo_name, key_id)
    local ok, status = fetch_json(
      base() .. "/repos/" .. repo_ref(owner, repo_name) .. "/keys/" .. key_id, "DELETE")
    if ok and (status == 204 or status == 200) then SetStatus(204, "No Content")
    elseif ok then respond_json(status, "Error", {})
    else respond_json(503, "Service Unavailable", {}) end
  end,
  ...
  get_repo_hooks = function(owner, repo_name)
    proxy_json(
      function(hooks)
        hooks = hooks or {}
        for i, h in ipairs(hooks) do hooks[i] = translate_harness_hook(h) end
        return hooks
      end,
      fetch_json(append_page_params(
  ...
  post_repo_hooks = function(owner, repo_name)
    proxy_json_created(translate_harness_hook,
      fetch_json(base() .. "/repos/" .. repo_ref(owner, repo_name) .. "/webhooks",
  ...
  patch_repo_hook = function(owner, repo_name, hook_id)
    proxy_json(translate_harness_hook,
      fetch_json(base() .. "/repos/" .. repo_ref(owner, repo_name) .. "/webhooks/" .. hook_id,
        "PATCH", translate_harness_hook_req(GetBody())))
  end,

  delete_repo_hook = function(owner, repo_name, hook_id)
    local ok, status = fetch_json(
      base() .. "/repos/" .. repo_ref(owner, repo_name) .. "/webhooks/" .. hook_id, "DELETE")
    if ok and (status == 204 or status == 200) then SetStatus(204, "No Content")
    elseif ok then respond_json(status, "Error", {})
    else respond_json(503, "Service Unavailable", {}) end
  end,
  ...
  get_repo_hook_config = function(owner, repo_name, hook_id)
    proxy_json(
      function(h) return (translate_harness_hook(h)).config or {} end,
      fetch_json(base() .. "/repos/" .. repo_ref(owner, repo_name) .. "/webhooks/" .. hook_id))
  end,
  ...
  post_repo_hook_test = function(owner, repo_name, hook_id)
    local ok, status = fetch_json(
      base() .. "/repos/" .. repo_ref(owner, repo_name) .. "/webhooks/" .. hook_id .. "/test", "POST")
    if ok and (status == 204 or status == 200) then SetStatus(204, "No Content")
    elseif ok then respond_json(status, "Error", {})
    else respond_json(503, "Service Unavailable", {}) end
  end,
