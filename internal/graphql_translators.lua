...
local GRAPHQL_SEARCH_COUNT_KEYS = {
  REPOSITORY = "repository",
  USER = "user",
  ISSUE = "issue",
}

-- graphql_search_resolver is global: builds a Query.search resolver from
-- backend-specific fetch/translate handlers while sharing the common
-- args/counts/connection scaffold.
function graphql_search_resolver(handlers) -- luacheck: globals graphql_search_resolver
  return function(_parent, args, ctx)
    local search_type, per_page, q = graphql_search_params(args)

    local nodes = {}
    local counts = { repository = 0, user = 0, issue = 0 }
    local handler = handlers[search_type]
    if handler then
      local items, total, err = handler.fetch(q, per_page)
      if not items then
        if ctx and err then
        ...
        for _, item in ipairs(items) do
          nodes[#nodes + 1] = handler.translate(item)
        end
        local key = GRAPHQL_SEARCH_COUNT_KEYS[search_type]
        if key then
          counts[key] = total or #nodes
        end
      end
    end

    return graphql_search_connection(nodes, counts)
  end
end
